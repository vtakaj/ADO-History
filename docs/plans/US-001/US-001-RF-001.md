...
### 2. メインモジュールの肥大化（解消済み）
- **ado-tracker.sh** は軽量なコマンドルーター
- 主要機能は `lib/` 配下へ分割済み

### 3. 設定とドキュメントの分散（継続対応）
- 設定は `.env` と `config` コマンドに統一
- ドキュメント整備は継続対応中
...
```
/
├── ado-tracker.sh         # メインエントリーポイント（軽量）
├── ado-tracker-backup.sh  # 旧スクリプト（参考）
├── lib/                   # モジュールライブラリ
├── tests/                 # テストスイート
├── docs/                  # ドキュメント
├── data/                  # 取得データ
├── work_records/          # 生成される作業記録
└── scripts/               # 補助スクリプト（必要に応じて追加）
```
...
### 現在のテスト状況
- **メインエントリーポイント**: 軽量なルーター
- **テストファイル**: unit 4件 / integration 4件 / helpers 3件
...
## Definition of Done
- [x] すべてのテストファイルがtests/配下に適切に分類されている
- [x] ado-tracker.sh が軽量なルーターとして運用されている
- [x] 主要機能が lib/ に分割されている
- [ ] 全テストが新構造で正常に実行される
